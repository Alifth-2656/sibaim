        ...
        <aside class="hidden md:flex md:flex-shrink-0 w-64 bg-[#1E4D9C] flex-col shadow-xl">
            ...
            <nav class="flex-1 px-4 py-6 space-y-1 overflow-y-auto hide-scrollbar">
                ...
                <div class="pt-4 pb-2 mt-4 border-t border-white/5">
                    ...
                    <a href="{{ route('improvement.inventory.index') }}"
                        class="flex items-center gap-4 px-4 py-3 rounded-xl transition-all duration-200 group
                        {{ request()->routeIs('improvement.inventory.*') ? 'bg-[#5EEAD4] text-[#1E4D9C] font-bold shadow-lg' : 'text-blue-100 hover:bg-white/10' }}">
                        <svg class="w-5 h-5 {{ request()->routeIs('improvement.inventory.*') ? 'text-[#1E4D9C]' : 'text-blue-300 group-hover:text-white' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div>
                    <a href="{{ route('improvement.kelola_barang.index') }}"
                        class="flex items-center gap-4 px-4 py-3 rounded-xl transition-all duration-200 group
                        {{ request()->routeIs('improvement.kelola_barang.*') ? 'bg-[#5EEAD4] text-[#1E4D9C] font-bold shadow-lg' : 'text-blue-100 hover:bg-white/10' }}">
                        <svg class="w-5 h-5 {{ request()->routeIs('improvement.kelola_barang.*') ? 'text-[#1E4D9C]' : 'text-blue-300 group-hover:text-white' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="flex flex-col flex-1 min-w-0 overflow-hidden">
            ...
            <main class="flex-1 overflow-y-auto p-8">
                ...
                <div class="mb-6 bg-green-100 border-l-4 border-green-500 text-green-700 p-4 rounded shadow-sm">
                    {{-- ✅ FIX: session('success') sekarang selalu string --}}
                    {{ is_string(session('success')) ? session('success') : 'Permintaan berhasil disimpan.' }}
                </div>
                @endif
            ...
            {{-- ✅ FIX: Struk dihapus dari sini — session('success') sudah string biasa --}}
            {{-- Kalau mau struk, implementasi terpisah via session('struk') --}}
        </div>
    </div>

    @stack('scripts')

    {{-- ✅ FIX: toggleNotif + click outside --}}
    <script>
